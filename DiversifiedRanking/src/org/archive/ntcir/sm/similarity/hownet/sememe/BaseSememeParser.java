package org.archive.ntcir.sm.similarity.hownet.sememe;
...
public abstract class BaseSememeParser implements HownetMeta, Similaritable {
	...
	public BaseSememeParser() throws IOException {
		...
		String sememeFile = JarDirectory.ROOT+"hownet/sememe/sememe.xml.gz";
		File file = new File(sememeFile);
		if(file.exists()){
			System.out.println(sememeFile);
		}
		//InputStream input = this.getClass().getClassLoader().getResourceAsStream(sememeFile);
		InputStream input = new GZIPInputStream(new FileInputStream(sememeFile));
		load(input);
	}
