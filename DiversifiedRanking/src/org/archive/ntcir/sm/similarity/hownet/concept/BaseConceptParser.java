...
import org.archive.JarDirectory;
import org.archive.ntcir.sm.similarity.hownet.HownetMeta;
 ...
 */
public abstract class BaseConceptParser implements HownetMeta{
	/** the logger */
	...
	private static void firstLoad() throws IOException{
		...
		String conceptFile = JarDirectory.ROOT+"hownet/concept/concept.xml.gz";
		///InputStream input = BaseConceptParser.class.getClassLoader().getResourceAsStream(conceptFile);
		InputStream input = new GZIPInputStream(new FileInputStream(conceptFile));
		load(input);
	}
