package org.archive.ntcir.sm.similarity.hownet.concept;
...
import org.archive.ntcir.sm.similarity.hownet.concept.BaseConceptParser;
import org.archive.ntcir.sm.similarity.hownet.concept.Concept;
import org.archive.ntcir.sm.similarity.hownet.concept.ConceptLinkedList;
import org.archive.ntcir.sm.similarity.hownet.sememe.BaseSememeParser;
import org.archive.ntcir.sm.similarity.hownet.sememe.XiaSememeParser;

class Vertex{
	private String word = null;
	private boolean reUpdate = false;
	public List<String> oovWords = null;
	public Collection<Concept> conceptList = null;
	//
	Vertex(String word, Collection<Concept> concepts){
		this.word = word;
		//
		if(null == concepts || concepts.size()==0){
			reUpdate = true;
		}else{
	...
	//
	public void setOOVWords(List<String> words){
		this.oovWords = words;
	}
	//
	public void updateConcepts(Collection<Concept> concepts){
		this.conceptList = concepts;
	}
	//
...
public class XiaConceptParser extends BaseConceptParser {
    ...
    public double getSimilarity(String word1, String word2) {
        ...
        if(BlankUtils.isBlank(concepts1)){
        	System.out.println("Blank concepts-1");
        }
        Collection<Concept> concepts2 = super.getConcepts(word2);
        if(BlankUtils.isBlank(concepts2)){
        	System.out.println("Blank concepts-2");
        }
    ...
    //
    public StringBuffer getSimilarity(Vector<String> wordVec, int i){
    	StringBuffer buffer = new StringBuffer();
    	//
    	String word_1 = wordVec.get(i);
    	Vertex vertex_1 = new Vertex(word_1, super.getConcepts(word_1));
    	if(vertex_1.needUpdate()){
    		vertex_1.setOOVWords(segmentOOV(word_1));
    	}
    	//
    	for(int j=i+1; j<wordVec.size(); j++){
    		String word_2 = wordVec.get(j);
    		Vertex vertex_2 = new Vertex(word_2, super.getConcepts(word_2));
    		if(vertex_2.needUpdate()){
    			vertex_2.setOOVWords(segmentOOV(word_2));
    		}
    		//
    		double similarity = 0.0;
    		//如果是blank，则说明是未登录词, 需要计算组合概念
        	if(vertex_1.needUpdate() && ! vertex_2.needUpdate()){
        		vertex_1.updateConcepts(autoCombineConcepts_x(vertex_1, vertex_2.conceptList));
        	}else if(!vertex_1.needUpdate() && vertex_2.needUpdate()){
        		vertex_2.updateConcepts(autoCombineConcepts_x(vertex_2, vertex_1.conceptList));
        	}else if(vertex_1.needUpdate() && vertex_2.needUpdate()){
        		//System.out.println("!!!");
        		vertex_1.updateConcepts(autoCombineConcepts_x(vertex_1, vertex_2.conceptList));
        		vertex_2.updateConcepts(autoCombineConcepts_x(vertex_2, vertex_1.conceptList));
        	...
        	//
        	// 两个for循环分别计算词语所有可能的概念的相似度
            for (Concept c1 : vertex_1.conceptList) {
                for (Concept c2 : vertex_2.conceptList) {
                    double v = getSimilarity(c1, c2);
            ...
            //
            buffer.append(word_1+"\t"+word_2+"\t"+similarity+"\n");
            //
    	}
    	//
    	return buffer;
    ...
    //
    public double getSimilarity_x(String word_1,String word_2){
    	//equal case;
    ...
    //
    private Collection<Concept> autoCombineConcepts_x(Vertex vertex, Collection<Concept> refConcepts){
    	ConceptLinkedList oovConcepts = new ConceptLinkedList();
    	//
    	if(null == vertex.oovWords){
    		return oovConcepts;
    	}
    	for(String concept_word: vertex.oovWords){
    ...
    public static void main(String []args){
    	...
    	/*
    	try{
    		XiaConceptParser conceptParser = new XiaConceptParser();
    		Collection<Concept> concepts = conceptParser.getConcepts("娱乐场");
    		int i=1;
    		for (Concept c : concepts) {
    			System.out.println("概念" + (i++) + ": " + c.toString());               
            }    		
    	}catch(Exception e){
    		e.printStackTrace();
    	}
    	*/
    	...
    	String w_1 = "李敏镐";
    	String w_2 = "吉他";
    	XiaConceptParser parser = XiaConceptParser.getInstance();
    	System.out.println(parser.getSimilarity(w_1, w_2));
    	System.out.println(parser.getSimilarity_x(w_1, w_2));
    }
}
